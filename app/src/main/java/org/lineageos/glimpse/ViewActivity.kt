                    ...
                    parsedIntent?.handle {
                        when (it) {
                            is ParsedIntent.ViewIntent -> {
                                viewModel.setMedias(it.medias)
                            }

                            is ParsedIntent.ReviewIntent -> {
                                viewModel.setAlbumRequest(it.albumRequest)
                                viewModel.setInitialMedia(it.initialMedia)
                                viewModel.setSecure(it.secure)
                            }
                    ...
                    when (it) {
                        ...
                        is RequestStatus.Success -> {
                            ...
                            val mediaPosition = viewModel.mediaPosition.value
                                ?: (viewModel.displayedMedia.value
                                    ?: viewModel.initialMedia.value)?.let { displayedMedia ->
                                    medias.indexOfFirst { media ->
                                        media.uri == displayedMedia.uri
                                    }.takeIf { idx -> idx != -1 }
                                } ?: 0
